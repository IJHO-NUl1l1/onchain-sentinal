...
- 8/14 기기2 (v2 완주) — **① 선택, 끝까지 돌림. v2 라이브 검증 완료.**
  ...
  **다음 기기가 할 일 (Flare만 남음):**
  - [ ] `/flare` 콘솔을 브라우저에서 직접 클릭해서 1~7막 전체 흐름 눈으로 확인
        (지금까지 검증은 전부 스크립트 직접 호출, 콘솔 UI 클릭 경로는 아직 미검증)
  - [ ] 데모 영상 촬영 — threshold 3bips 권장(회색지대 도달이 제일 빠르고 안정적이었음),
        즉시방어(2배 초과) 경우는 화면에 안 담고 말로만 설명하기로 확정(agent 역할이 핵심이라서)
  - [ ] `README-flare-draft.md`를 `submission/flare` 브랜치의 루트 `README.md`로
  - [ ] 브랜치 동결 + 태그, 제출
  - [ ] Flare Summer Signal 실제 제출 폼을 아직 한 번도 못 봤음 — 폼 화면 확인되면 필드별로
        채울 내용 정리 필요(KeeperHub 때 DoraHacks 폼처럼)
