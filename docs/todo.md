...
1. [x] **`README-flare-draft.md` 커밋·푸시 완료** (repo 루트) — 6단계 tx 표까지 다 채워져 있음.
   `submission/flare` 동결 시 이 파일 내용을 브랜치 루트 `README.md`로 반영할 것
2. [x] **`/flare` 콘솔 브라우저 클릭 검증 완료 (8/14)** — threshold 5bips로 1~7막 전부 실제
   클릭으로 완주: 예치(115)→정책→에스컬레이션(6bips)→**Claude 실제 판단**(`INCREASE_MONITORING`,
   "1bip만 더 움직이면 도로 threshold 밑"까지 스스로 계산)→상태 불변 확인→**잠기지 않았으니
   인출 정상 성공**(115→105). 버그 없음, 그대로 촬영 가능한 상태.
3. [ ] **Flare 데모 영상 촬영** — 위 실제 브라우저 결과를 그대로 재현(또는 이번 결과 자체가
   녹화본이었다면 이걸 그대로 사용)
