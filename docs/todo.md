...
**✅ KeeperHub 트랙 — 완전히 끝남.** 코드·검증·촬영에 이어 **8/13에 DoraHacks 폼 제출까지 완료
확인됨(8/14 사용자 확인).** 더 이상 할 일 없음.

**✅ Flare 트랙 — 완전히 끝남.** v2(실자산 커스터디) 라이브 검증, 촬영, README, 브랜치 동결·태그
전부 완료(8/15). 제출 URL: `https://github.com/IJHO-NUl1l1/onchain-sentinel/tree/submission/flare`
(태그 `flare-submission-2026-08-15`). **남은 건 실제 제출 폼에 이 URL 붙여넣는 것뿐.**

1. [x] README 완성, `submission/flare` 브랜치 루트 반영 완료
2. [x] `/flare` 콘솔 브라우저 클릭 검증 완료
3. [x] **데모 영상 촬영 완료 (8/15)** — threshold 5bips, 실제 촬영본 tx: 예치 `0x71874ee5…` →
   정책 `0xc67a6b9b…`(anchor 1.004423) → 에스컬레이션 `0x52640ebc…`(6bips) → **Claude 실제 판단**
   (`INCREASE_MONITORING`, "6/10bips, 얕은 회색지대, 단일 피드 업데이트로도 넘을 수 있는 폭"이라고
   스스로 계산) → 상태 불변 확인 → 안 잠겨서 정상 인출 `0x8a273540…`. README Proof 섹션에 그대로 반영.
   ⚠️ 중간에 threshold 5로 재시도하다 한 번은 가격이 한 번에 10bips까지 튀어서 즉시방어(컨트랙트
   자체 잠금)로 간 적 있음 — 이것도 실제 결과, README에 별도 표로 보존해둠(회색지대 경로가
   최종 촬영본이 됨)
4. [x] **`submission/flare` 브랜치 동결 + 태그 완료 (8/15)**
...
- 8/14 기기1(커스터디) — **⚠️ 절반에서 멈춤. 이어받기 전에 전부 읽을 것.**

  **왜 손댔나.** 사용자가 콘솔로 Flare 5막을 완주시킨 뒤 물었다: "`LOCK_POSITION`이 실제로
  어떤 온체인 자산이나 데이터를 건드는 거냐?" 확인해보니 v1 `SentinelVault`는 **자산을 전혀
  보유하지 않는다** — `policy.isLocked = true` + 이벤트가 전부다. Flare 바운티가
  **Interoperable Asset Products**라 "자산 제품인데 자산을 안 건드린다"는 약점이라 판단,
  사용자가 커스터디 추가를 선택했다.

  **한 것 (커밋됨):**
  - `SentinelVault.sol`에 `deposit()`(payable) / `withdraw(uint256)` / `balances` 매핑 추가.
    핵심은 `withdraw`가 **정책이 잠겨 있으면 `PositionLocked`로 revert**한다는 것 —
    방어가 플래그가 아니라 자금의 출구를 막는 조건이 된다. `unlock()`은 원래 있던 걸 그대로 둠
    (사용자가 스스로 풀 수 있음 = 자산을 인질로 안 잡는다).
    기존 로직(`setPolicy`/`checkAndExecute`/`agentRespond`)은 **한 줄도 안 건드림.**
  - 컴파일 통과(solc 0.8.25, cancun) → **Coston2 재배포 완료: `0x1288516DcE1642952d1e3eB79504F496edb38D31`**
  - 코드 4곳의 주소를 v2로 교체: `app/executors/flare.ts`, `app/app/_components/flare-console.tsx`,
    `contracts/scripts/demo-provision-and-check.ts`, `.env.example`
  - `flare.ts`에 헬퍼 추가: `readVaultBalance` / `depositToVault` / `tryWithdraw`(잠기면 실패를
    예외가 아니라 결과로 돌려줌 — 화면에서 "막혔다"를 보여주려고) / `unlockPosition`
  - `next build` + `tsc` 통과

  **⛔ 안 한 것 (여기서 멈췄다):**
  1. **v2를 온체인에서 한 번도 안 돌려봤다.** 예치·인출·잠금 동작 **전부 미검증**.
     현재 v2 상태: 정책 없음, 잔고 0 C2FLR.
  2. **콘솔에 예치/인출 막이 없다.** 지금 `/flare`를 돌리면 v2에 정책만 새로 깔고 예전과 똑같이
     5막이 흐른다 — **"자금이 실제로 동결됐다"는 장면은 아직 UI에 존재하지 않는다.**
  3. 문서 일부에 v1 주소가 남아 있다(README.md, todo.md 과거 로그, architecture.md 일부) —
     과거 실증 기록은 **실제로 v1에서 나온 것이라 그대로 두는 게 맞다.** §3에 두 배포본 비교표를 새로 넣었다.

  **⚠️ 지금까지의 Flare 실증 tx는 전부 v1에서 나온 것이다** (`setPolicy 0x270ad4a0…`,
  `agentRespond 0xcc8092c9…`, `checkAndExecute 0xf5e238c2…`). v2로 제출하려면 그 증빙을
  v2에서 다시 만들어야 한다.

  **다음 기기의 선택 (마감 8/15 04:59, 8/14 오전 기준 약 18시간):**

  **① v2로 완주** — 남은 일: ⓐ`depositToVault`로 C2FLR 예치(지갑에 193개 있음, 10개면 충분)
  ⓑ전 과정 라이브 검증(예치→정책→하락→에스컬레이션→`LOCK_POSITION`→**인출 시도가 revert되는지**)
  ⓒ콘솔에 예치 막 + "인출 시도" 막 추가 ⓓ촬영. **가장 강한 그림이 나온다** —
  "AI 판단으로 실제 자금이 잠겼다"를 revert 화면으로 증명. 예상 2~3시간.

  **② v1으로 롤백** — 4개 파일에서 주소를 `0xBf5778109e894b7C093D91B8a7518c95Fe74c3EF`로
  sed 치환하면 끝. v1은 이미 라이브 검증·콘솔 검증이 다 끝나 있어서 **바로 촬영 가능**.
  대신 "자산을 안 건드린다"는 약점을 README에 정직하게 쓰고 로드맵으로 남긴다.
...
- 8/14 기기2 (v2 완주) — **① 선택, 끝까지 돌림. v2 라이브 검증 완료.**
  ...
  **✅ KeeperHub 제출 확인 완료 (8/14) — 8/13에 이미 제출됨.** KeeperHub 트랙은 더 이상 할 일 없음.

  **✅ vault에 100 C2FLR 추가 예치 (8/14, 실행 지갑 기존 잔고에서 바로)** — tx `0x956b5ad8…`,
  vault 잔고 현재 105 C2FLR. 개인 지갑 경유 불필요했음(실행 지갑에 파우셋 잔고 충분).

  **✅ 콘솔 브라우저 클릭 검증도 끝남 (8/14, 뒤 항목 참조)** — threshold 5bips로 1~7막 실제
  클릭 완주, 버그 없음. 남은 건 촬영·브랜치·제출뿐. 최신 상태는 맨 위 "▶ 다음 기기가 할 일"
  섹션 참조(이 로그 안 목록은 갱신 안 함 — 과거 기록이라 그대로 둠).
