...
- **UI**: ~~터미널 녹화로 대체~~ **8/14 재변경 — 웹 UI 패널을 새로 만든다.** 라이브 사이클이
  이미 한 번 완주됐고 마감까지 24시간 넘게 남아서, 시간 압박이 풀렸다고 판단해 원래 계획(새
  웹 패널)으로 되돌림. `run-console.tsx`의 5막 구조·스타일을 그대로 재사용하되 Flare 데이터로
  배선. **Flare 기술을 쓴 지점(FTSOv2 실측값, SentinelVault 3단 판단, `checkAndExecute` 이벤트)이
  화면에서 눈에 띄게 강조돼야 한다** — KeeperHub 콘솔이 "이건 실제 Aave 호출이다"를 라벨로
  못박았던 것과 같은 방식.
