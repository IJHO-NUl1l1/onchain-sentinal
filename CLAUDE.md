...
**밖으로 노출되는 것은 전부 영어.** 심사위원과 사용자가 보는 면이다:
- README, 제출 서면
- UI 문자열 전부 (버튼·상태·에러 메시지·빈 상태), 페이지 `metadata`
- **KeeperHub에 생성되는 워크플로우 `name`/`description`** — 남의 대시보드에 그대로 뜬다
- 데모 스크립트 콘솔 출력 — **영상에 터미널이 찍힌다**
- 커밋 메시지

**한국어로 둬도 되는 것:** 코드 주석, `docs/*.md` 내부 문서. 우리 작업용이고 심사에서 소스 주석까지 파고들진 않는다.

새 UI 문자열이나 로그를 추가할 때 **처음부터 영어로 써라.** 나중에 번역하면 반드시 하나씩 샌다.
